...
func init() {
    ...
    size := im.Bounds().Size()
    mask := image.NewRGBA( image.Rect(0, 0, size.X/font_scale, size.Y/font_scale ) )
    for y := 0; y < size.Y; y+= font_scale {
        for x := 0; x < size.X; x+= font_scale {
            r, _, _, _ := im.At(x, y).RGBA()
            if r > 0 {
                mask.Set(x/font_scale, y/font_scale, color.Opaque)
            }
        }
    }
    fontMask = mask
...
func DrawCenteredText(dst draw.Image, dx, dy int, text string, c color.Color) {
    ...
    for i, row := range rows {
        x := dst.Bounds().Size().X/2 - len(row)* FONT_SIZE / 2
        y := dst.Bounds().Size().Y/2 - len(rows)*FONT_SIZE/2 + i*FONT_SIZE
...
func DrawCharacter(dst draw.Image, x, y int, ch byte, c color.Color) {
    ...
    cx := ((int(ch)-32)&0xF) * FONT_SIZE
    cy := ((int(ch)-32)>>4) * FONT_SIZE
    r := image.Rect(x, y, x+FONT_SIZE, y+FONT_SIZE)
    src := &image.Uniform{c}
    sp := image.Pt(cx, cy)
    draw.DrawMask(dst, r, src, sp, fontMask, sp, draw.Src)  // draw.Over -> draw.Src
}
